...
`install-miscellany.bash` runs from macOS. It clones or updates the repo on the remote host, copies your secrets and certs, and runs the Helm/kubectl deployment.
...
```bash
ansible/run-playbook.bash --config hosts/miscellany/host.yml
```

Then deploy:

```bash
hosts/miscellany/scripts/install-miscellany.bash --config hosts/miscellany/host.yml
```

## Updating config

After the initial install, `run-miscellany-upgrade.bash` runs directly on the host. SSH in and run it from the repo:

```bash
ssh <user>@<host>
cd ~/stuff/homelab-provisioner
hosts/miscellany/scripts/run-miscellany-upgrade.bash
```
